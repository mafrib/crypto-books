function createTreemap(selector, data, mode = 'category') {
    ...
    function zoom(node) {
        ...
        if (node.children) {
            const newRoot = d3.hierarchy({
                name: node.data.name,
                children: node.children.map(d => d.data)
            })
            .sum(d => d.value)
            .sort((a, b) => b.value - a.value);
            ...
            if (node.parent === null) {
                ...
                `);
                path.select("span").on("click", (event) => {
            ...
            const cell = group.selectAll("g.cell")
                .data(nodes.length ? nodes : root.children, d => d.data.name);
            ...
            cellUpdate.select("text")
                ...
                .on("end", function(d) {
                    ...
                    // Final overflow check after truncation
